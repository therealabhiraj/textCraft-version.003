...
class Slider extends Component {
  ...
  render() {
    ...
    return (
      <div
        className="hidden w-2/3 sm:block min-h-[420px] h-fit relative  px-2 py-3 bg-zinc-800 rounded-lg "
        onMouseEnter={this.stopAutoplay}
        onMouseLeave={this.startAutoplay}
      >
        {/* Main Slide */}
        <div className="min-h-[420px] relative rounded-lg text-center  ">
        ...
        <img
          ...
          <button
            className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-70 text-white text-3xl p-2 cursor-pointer"
        ...
        <div className=" flex items-center min-h-28 justify-center mt-4">
          {/* Scroll Buttons */}
          <button
            className={`bg-black bg-opacity-70 text-white text-3xl p-2 cursor-pointer ${thumbnailScrollIndex === 0 ? 'opacity-50' : ''}`}
          ...
          <div className="flex overflow-hidden mx-2  h-full">
            {this.slideData.map((slide, index) => (
              <img
                ...
                src={slide.src}
                alt={slide.headline}
                className={`w-[100px] h-[100px] object-cover mx-1 cursor-pointer opacity-70 transition-opacity duration-300 ${index === current ? 'opacity-100' : ''}`}
          ...
          <button
            className={`bg-black bg-opacity-70 text-white text-3xl p-2 cursor-pointer ${thumbnailScrollIndex >= this.slideData.length - 6 ? 'opacity-50' : ''}`}
